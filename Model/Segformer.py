...
class IRTDefectDetector(nn.Module):
    ...
    def forward(self, x_img):
        feat, _ = self.backbone(x_img)  # 不再用 low feature
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for temp, label in train_loader:
            temp, label = temp.to(device), label.to(device)
        ...
        if val_miou_epoch > best_miou:
            ...
            torch.save(model.state_dict(), r"..\Weights\best_model_pure.pth")
            print(f"✅ Saved BEST model at epoch {epoch + 1}")

        # 每20个epoch保存
        if (epoch + 1) % 20 == 0:
            torch.save(model.state_dict(), f"..\Weights\epoch_{epoch + 1}_pure.pth")
    ...
    save_path = r"..\Weights\metrics_full_delamination_pure.xlsx"
    df.to_excel(save_path, index=False)

    print("Saved metrics to:", save_path)
    torch.save(model, r"..\Weights\model_delamination_pure.pt")